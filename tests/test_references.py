...
def _get_bib_files():
    return list(CONTENT_DIR.rglob("*.bib"))
...
def test_bib_files_exist():
    assert len(BIB_FILES) > 0, "No .bib files found in content/"
...
def test_bib_files_have_content():
    ...
    for bib_path in BIB_FILES:
        ...
        if not text:
            empty.append(str(bib_path.relative_to(CONTENT_DIR.parent)))
    if empty:
...
def test_bib_files_have_citations():
    ...
    for bib_path in BIB_FILES:
        ...
        if "@" not in text:
            no_citations.append(str(bib_path.relative_to(CONTENT_DIR.parent)))
    if no_citations:
    ...
    for module_dir in sorted(CONTENT_DIR.glob("module*")):
        ...
        for lesson_dir in sorted(lessons_dir.glob("lesson*/")):
            if lesson_dir not in lesson_bib_map:
                missing.append(str(lesson_dir.relative_to(CONTENT_DIR.parent)))
    if missing:

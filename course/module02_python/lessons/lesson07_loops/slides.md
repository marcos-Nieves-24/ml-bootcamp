...
## Diapositiva 6: Break and Continue
```python
for i in range(10):
    if i == 3:
        continue  # skip 3
...
## Diapositiva 8: Loop else Clause
```python
for x in data:
    if condition(x):
        break
...
## Diapositiva 9: enumerate() and zip()
```python
